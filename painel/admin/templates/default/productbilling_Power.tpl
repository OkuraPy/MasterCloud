            ...
            <div style="">
                ...
                *}
                ...
                <table width="100%" cellspacing="0" cellpadding="6" border="0">
                    ...
                    <tr>
                        <td >kW/h Price</td>
                        <td>{$currency.sign} <input type="text" class="inp" size="4"  value="{if $configuration.kwh}{$configuration.kwh|price:$currency:false}{else}{0.00|price:$currency:false}{/if}" name="config[kwh]" /> {$currency.code}</td>
                    </tr>
                    <tr>
                        <td >Setup fee</td>
                        <td>{$currency.sign} <input type="text" class="inp" size="4"  value="{if $configuration.SetupFee}{$configuration.SetupFee|price:$currency:false}{else}{0.00|price:$currency:false}{/if}" name="config[SetupFee]" /> {$currency.code}</td>
                    </tr>
                    <tr>
                        <td>Fixed recurring fee <a title="Fixed amount, recurring charge for service" class="vtip_description"></a></td>
                        <td>{$currency.sign} <input type="text" class="inp" size="4" value="{if $configuration.RecurringFee}{$configuration.RecurringFee|price:$currency:false}{else}{0.00|price:$currency:false}{/if}" name="config[RecurringFee]" /> {$currency.code}</td>
                    </tr>

    ...
    <div class=" menubar">
        ...
        *}{if !$forbidAccess.deleteInvoices}<a class="deleteInvoice menuitm {if $invoice.status!='Draft' && $invoice.status!='Recurring'}menul{/if}" name="delete" ><span style="color:#FF0000">{$lang.Delete}</span></a>{/if}
        ...
        <ul id="hd2_m" class="ddmenu">
            ...
            {/if}
            {if $proforma && $invoice.paid_id==''  && !$invoice.bulkinvoice}
                <li class="act_unpaid {if $invoice.status!='Paid' ||( $invoice.datepaid!='' &&  $invoice.datepaid!='0000-00-00 00:00:00')}disabled{/if}"><a href="GenerateFinalId">Generate Final ID</a></li>
            {/if}
    ...
    <form action="" method="post" id="itemsform">
        <table class="invoice-table" width="100%" border="0" cellpadding="0" cellspacing="0">
            ...
            <tbody id="new-item-invoice">
                ...
                {if $hasitemqueue}
                    <tr><td colspan="7">
                            <div class="p6 secondtd" style="text-align:center;margin-bottom:7px;padding:15px 0px;">
                                This customer have some items in invoice queue, would you like to add them here?<br/>
                                <a class="new_control greenbtn" href="#" onclick="{literal}if(confirm('Are you sure? Invoice queue will be emptied for this client')){ $('#itemqueue').click();return false}else{return  false;}{/literal}" ><span>Add items from queue</span></a>
                                <input type="submit" value="Save Changes" id="itemqueue" style="display:none" name="itemqueue">
                            </div>
                        </td></tr>

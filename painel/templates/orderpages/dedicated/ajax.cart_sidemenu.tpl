...
<div class="bC">
			...
			{foreach from=$cart_contents[1] item=cstom2 name=foo}
				{foreach from=$cstom2 item=cstom}
					...
					<li class="i2 {if $alter%2==0}odd{/if}"><span class="s1"></span>
						<span class="s2">{$cstom.name} {if $cstom.qty>=1}x {$cstom.qty}{/if}</span>
	...
	<p class="price">
		...
		{if $product.paytype!='Once' && $product.paytype!='Free' && $progress<5}
			<a href="#" class="code" onclick="$(this).hide();$('#cycleform').show();return false;" style="display:block">- {$lang.changebillingcycle}</a>
		...
		{if count($currencies)>1}
			<a href="#" class="code" onclick="$(this).hide();$('#currform').show();return false;" style="display:block">- {$lang.changecurrency}</a>
		...
		{if $subtotal.coupon}
		{else}
			<a href="#" class="code" onclick="$(this).hide();$('#promocode').show();return false;" style="display:block">- {$lang.usecoupon}</a>

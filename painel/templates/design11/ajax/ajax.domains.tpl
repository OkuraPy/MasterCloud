    ...
    {if $domains}
        {foreach from=$domains item=domain name=foo}
            <tr class="{if $domain.status == 'Expired'}expired{elseif $domain.status == 'Active' && $domain.daytoexpire < 60 && $domain.daytoexpire >= 0} nearexpire{/if}" >
                ...
                <td class="visible-desktop" style="vertical-align:central">{if !$domain.date_created || $domain.date_created == '0000-00-00'}-{else}{$domain.date_created|dateformat:$date_format}{/if}</td>
                <td>
                ...
                <td class="visible-desktop" style="width: 50px">
                    {if $domain.status=='Active' || $domain.status == 'Expired'}

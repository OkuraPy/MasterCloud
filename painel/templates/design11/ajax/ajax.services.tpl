...
{elseif ($action=='services'  || $action=='reseller' || $action=='servers' || $action=='accounts') && $services}
    {foreach from=$services item=service name=foo}
        <tr>
                ...
                <td class="p2 overflow-elipsis">
                    <div>
                        <a href="{$ca_url}clientarea/{$action}/{$service.slug}/{$service.id}/" class="roll-link">
                            <span data-title="{$service.name}">{$service.name}</span> <span class="visible-desktop">{include file='../common/tpl/inlinelabel.tpl'}</span>
                        </a>
                        {if $service.domain!=''}
            ...
            <td class="visible-desktop">{$service.total|price:$currency}</td>
            <td class="visible-desktop">{$lang[$service.billingcycle]}</td>
...
{elseif ($action=='vps') && $services}
    {foreach from=$services item=service name=foo}
        <tr class="styled-row">
            ...
            <td class="visible-desktop">{$service.ip}</td>
            <td>{if $service.next_due!=0}{$service.next_due|dateformat:$date_format}{else}-{/if}</td>

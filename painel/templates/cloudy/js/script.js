...
$(function() {
    ...
    $(document).on('click.dropdown.data-api','[data-toggle=dropdown][data-contener]', function(){
        var self = $(this),
            parent = self.parent();

        parent.clone(true).css({position:'absolute',top:0,left:0}).addClass('not-cart').data('dropdown-clone', parent).appendTo(self.data('contener')).children().each(function(i){

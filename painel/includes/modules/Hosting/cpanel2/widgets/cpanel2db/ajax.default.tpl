...
{else}	
	{if $listentrys}
		...
		<tr {counter}{if $even % 2 !=0}class="even"{/if}>
			...
			<td align="center">
                            <a class="adminlink" data-db="{$entry.db}" 
                               href="{$access.url}&goto_uri={$access.token}/3rdparty/phpMyAdmin/index.php?db={$entry.db}#PMAURL-1:db_structure.php?db={$entry.db}" 
                               target="_blank" onmouseup="regenerate();">{$widget_lang.db_admin}</a> | 

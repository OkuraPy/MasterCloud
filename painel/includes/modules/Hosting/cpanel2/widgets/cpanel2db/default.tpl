                ...
                <div class="section">
                    {literal}
                        <script type="text/javascript">
                            ...
                            function regenerate() {
                                var adm = $('.adminlink');
                                adm.bind('click.adm', function() {
                                    return false;
                                }).addClass('disabled').attr('href', window.location.url);
                                $.post(window.location.url, {regenerate: 1}, function(data) {
                                    adm.unbind('click.adm mouseup.adm').removeClass('disabled')
                                    if (!data)
                                        return false;

                                    adm.each(function() {
                                        var self = $(this);
                                        self.attr('href', data.url + '&goto_uri=' + data.token + '/3rdparty/phpMyAdmin/index.php?db='
                                            + self.data('db') + '#PMAURL-1:db_structure.php?db=' + self.data('db'))
                                    })
                                })
                            }
                                ...
                                $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        if (thisvar.attr('title') == 'Remove')
                                            ...
                                            thisvar.attr('src', '{/literal}{$widgeturl}{literal}trash.gif').attr('title', 'Remove').attr('alt', 'Remove');
